...
class LlavaMetaForCausalLM(ABC):
    ...
    images, image_sizes=None
        ...
        if images.ndim == 5:
            ...
            # print(f"have mutiple imagegs {images.shape}")
            concat_images = torch.cat([image for image in images], dim=0)
            image_features = self.encode_images(concat_images)
            split_sizes = [image.shape[0] for image in images]
            image_features = torch.split(image_features, split_sizes, dim=0)
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            # print(f"image feature shape : {cur_image_feature.shape}")
            # cur_labels_noim = []
            for i in range(len(image_token_indices) - 1):
                cur_input_ids_noim.append(cur_input_ids[image_token_indices[i]+1:image_token_indices[i+1]])
                # cur_labels_noim.append(cur_labels[image_token_indices[i]+1:image_token_indices[i+1]])
            ...
            for i in range(num_images + 1):
                ...
                # cur_new_labels.append(cur_labels_noim[i])
                if i < num_images:
                    cur_image_features = cur_image_feature[cur_image_idx]
                    cur_image_idx += 1
                    cur_new_input_embeds.append(cur_image_features)
                    # cur_new_labels.append(torch.full((cur_image_features.shape[0],), IGNORE_INDEX, device=cur_labels.device, dtype=cur_labels.dtype))
            
            # for i in range(len(cur_new_input_embeds)):
            #     print(cur_new_input_embeds[i].shape, end=" ")
            cur_new_input_embeds = [x.to(self.device) for x in cur_new_input_embeds]
        ...
        if _labels is None:
            ...
            # have to change to labels when want to change to 1 * 76
            # new_labels = new_labels_padded
            new_labels = labels
        ...
        # print(new_labels.shape, new_labels.dtype) # 不知道为什么会转成bfloat
        return None, position_ids, attention_mask, past_key_values, new_input_embeds, new_labels
