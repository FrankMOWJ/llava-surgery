...
def eval_model(args):
    ...
    for (input_ids, image_tensor, image_sizes), line in tqdm(zip(data_loader, questions), total=len(questions)):
        ...
        if args.mode == "text":
            ...
            outputs = output_ids.logits
            # print(len(outputs[0][0].cpu().detach().numpy().tolist()))
            str_output = str(outputs[0][0].cpu().detach().numpy().tolist())
            ans_file.write(json.dumps({"question_id": idx,
                                    "prompt": cur_prompt,
                                    "text": str_output,
                                    "answer_id": ans_id,
